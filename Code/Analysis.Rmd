...
monthly.cd.plot <- ggplot(cd.monthly_post1990.interp, aes(x = as.Date(Month.Year), y = Conc))+
   ...
   theme(legend.position = "none")+
   labs(x= "Date", y = "Conc of metal (mg/kg wet wt)")
monthly.pb.plot <- ggplot(pb.monthly_post1990.interp, aes(x = as.Date(Month.Year), y = Conc))+
   geom_line(color = my.colors[2])+
   geom_point(aes(shape = as.factor(estimated)), color = my.colors[2])+
   scale_shape_manual(values = c(16,1))+
   coord_cartesian(ylim=c(0, 10))+
...
ancova.cd <- lm(log(Value.mgperkg+1) ~ MYEAR + Country * Longitude + Latitude, 
                data = cd.countries)
summary(ancova.cd)


ancova.pb <- lm(log(Value.mgperkg+1) ~ MYEAR + Country * Longitude + Latitude, data = pb.countries)
summary(ancova.pb)
